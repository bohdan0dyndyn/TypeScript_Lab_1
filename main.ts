...
function triangle(
  ...
  if (
    !VALID_TYPES.includes(type1 as ElementType) ||
    !VALID_TYPES.includes(type2 as ElementType)
  ) {
    console.log("Invalid type(s). Please re-read the instructions.");
    return "failed";
  }

  const t1 = type1 as ElementType;
  const t2 = type2 as ElementType;
  ...
  const angleTypes: ElementType[] = ["adjacent angle", "opposite angle", "angle"];
  ...
  const get = (t: ElementType): number => (t === t1 ? val1 : val2);
  ...
  if (t1 === "leg" && t2 === "leg") {
    ...
    if (a >= c) return "Leg must be shorter than hypotenuse";
    b = Math.sqrt(c * c - a * a);
    ...
    (t1 === "hypotenuse" && t2 === "opposite angle") ||
    ...
    (t1 === "hypotenuse" && t2 === "adjacent angle") ||
